...
num_barras = 4;
b_shunt = 1.2i;
potencias_especificadas = [1;1;1;1;1]%[p1,p2,p3,q2,q3]
vetor tensao = [1;1;1;1];
vetor_angulo = [0;0;0;0];
vetor_v_theta = [vetor_angulo(1);vetor_angulo(2);vetor_angulo(3);vetor_tensão(2);vetor_tensão(3)];%[theta1,theta2,theta3,v2,v3]
...
for i = 1:num_barras
    ...
    for j = 1:num_barras
      p_calc(i) = p_calc(i)+ vetor_tensao(i)vetor_tensão(j) * (parte_real(i,j) * cos(vetor_angulo(i) - vetor_angulo(j)) + parte_imaginaria(i,j) * sin(vetor_angulo(i) - vetor_angulo(j)));
    end
end
%Potencias reativas
...
for i = 1:num_barras
    % Cálculo da potência ativa para a barra i
    for j = 1:num_barras
      q_calc(i) = q_calc(i)+ vetor_tensao(i)vetor_tensão(j) * (parte_real(i,j) * sin(vetor_angulo(i) - vetor_angulo(j)) - parte_imaginaria(i,j) * cos(vetor_angulo(i) - vetor_angulo(j)));
    end
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%Atualizando o vetor tensao e angulo com os elementos ddo vetor theta_v%%%%%%%%%%%%%%%%%%%%%
